CXX=g++-9
CXXFLAGS=-O3 -Wall -Wextra -std=c++17 -I Shared
MAINFILE=main.cpp

KRUSKAL_NAIVE=KruskalNaive
KRUSKAL_UNION_FIND=KruskalUnionFind
KRUSKAL_UNION_FIND_COMPRESSED=KruskalUnionFindCompressed
PRIM_BINARY_HEAP=PrimBinaryHeap
PRIM_K_HEAP=PrimKHeap

all: ${KRUSKAL_NAIVE} ${KRUSKAL_UNION_FIND} ${KRUSKAL_UNION_FIND_COMPRESSED} ${PRIM_BINARY_HEAP} ${PRIM_K_HEAP}

${KRUSKAL_NAIVE}:
	${CXX} ${CXXFLAGS} ${KRUSKAL_NAIVE}/${MAINFILE} -o ${KRUSKAL_NAIVE}.out

${KRUSKAL_UNION_FIND}:
	${CXX} ${CXXFLAGS} ${KRUSKAL_UNION_FIND}/${MAINFILE} -o ${KRUSKAL_UNION_FIND}.out

${KRUSKAL_UNION_FIND_COMPRESSED}:
	${CXX} ${CXXFLAGS} ${KRUSKAL_UNION_FIND_COMPRESSED}/${MAINFILE} -o ${KRUSKAL_UNION_FIND_COMPRESSED}.out

${PRIM_BINARY_HEAP}:
	${CXX} ${CXXFLAGS} ${PRIM_BINARY_HEAP}/${MAINFILE} -o ${PRIM_BINARY_HEAP}.out
	
${PRIM_K_HEAP}:
	${CXX} ${CXXFLAGS} ${PRIM_K_HEAP}/${MAINFILE} -o ${PRIM_K_HEAP}.out

testall:
	./testall.sh

testall_explicit:
	./test.sh ${KRUSKAL_UNION_FIND}.out
	./test.sh ${KRUSKAL_UNION_FIND_COMPRESSED}.out
	./test.sh ${PRIM_BINARY_HEAP}.out
	./test.sh ${PRIM_K_HEAP}.out
# TODO: Test KruskalNaive excluded due to its runtime.
# ./test.sh ${KRUSKAL_NAIVE}.out

report:
	cd report; make pdf2

.PHONY: all ${KRUSKAL_NAIVE} ${KRUSKAL_UNION_FIND} ${KRUSKAL_UNION_FIND_COMPRESSED} ${PRIM_BINARY_HEAP} ${PRIM_K_HEAP} report
