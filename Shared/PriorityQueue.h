...
class PriorityQueue : private BinaryHeap<T, is_heap_on_init> {
   ...
   public:
    ...
    explicit PriorityQueue(const PriorityQueue<Key, T>& rhs) = delete;
    PriorityQueue& operator=(const PriorityQueue<Key, T>& rhs) = delete;
...
// create a Priority Queue based on a Min Heap
template <typename Key, typename Value, bool is_heap_on_init = false>
PriorityQueue<Key, Value, is_heap_on_init> make_min_priority_queue(std::vector<Key>&& keys, std::vector<Value>&& inputs) {
    return PriorityQueue<Key, Value, is_heap_on_init>(std::move(keys), std::move(inputs), [](auto& key_map) {
...
// create a Priority Queue based on a Max Heap
template <typename Key, typename Value, bool is_heap_on_init = false>
PriorityQueue<Key, Value, is_heap_on_init> make_max_priority_queue(std::vector<Key>&& keys, std::vector<Value>&& inputs) {
    return PriorityQueue<Key, Value, is_heap_on_init>(std::move(keys), std::move(inputs), [](auto& key_map) {
        return [&key_map](auto&& a, auto&& b) -> bool {
            return key_map.at(a) < key_map.at(b);
        };
