...
/**
 * VertexWeightLink represents a node and the weight associated to one of its linked nodes.
 */
template <typename Label, typename Weight>
struct VertexWeightLink {
	...
	explicit VertexWeightLink(const Label& vertex, const Weight& weight) noexcept :
		vertex(vertex),
		weight(weight) { }
};
...
namespace custom_hash {
	...
	// commutative hash functor for VertexWeightLink<Label, Weight>
	struct vertex_weight_hash {
		template <class Label, class Weight>
		std::size_t operator()(const VertexWeightLink<Label, Weight>& vertex_weight_link) const {
			const auto& vertex = vertex_weight_link.vertex;
			return std::hash(vertex);
		}
	};
}
...
class AdjacencyMapGraph {
	...
	/**
	 * Initializes the graph starting from a vector of edges. The given number of vertexes is used to
	 * efficiently preallocate memory and void exprensive and multiple rehashes.
	 * Time: O(n + m)
	 * Space: O(n + m)
	 */
...
template <typename Label, typename Weight>
inline void AdjacencyMapGraph<Label, Weight>::init(const std::vector<Edge<Label, Weight>>& edge_list, const size_t n_vertex) {
	// preallocate memory
...
inline std::vector<Label> AdjacencyMapGraph<Label, Weight>::get_vertexes() const {
	...
	std::transform(adj_map.cbegin(), adj_map.cend(), std::back_inserter(vertexes), [](const auto& map_entry) {
...
template <typename Label, typename Weight>
inline const std::unordered_set<Edge<Label, Weight>, custom_hash::edge_hash>& AdjacencyMapGraph<Label, Weight>::get_edges() const {
	return edge_set;
}

template <typename Label, typename Weight>
template <class Comparator>
inline std::vector<Edge<Label, Weight>> AdjacencyMapGraph<Label, Weight>::get_sorted_edges(Comparator&& comparator) const {
	...
	std::sort(sorted_edges.begin(), sorted_edges.end(), [comp{ std::move(comparator) }](const auto& l, const auto& r) {
...
template <typename Label, typename Weight>
inline const std::unordered_map<Label, Weight>&
AdjacencyMapGraph<Label, Weight>::adjacent_vertexes(const Label& vertex) const {
